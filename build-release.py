...
def parse_args():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/cyberyen/cyberyen', help='Specify the URL of the repository. Default is %(default)s')
...
def setup():
    ...
    if not os.path.isdir('gitian.sigs.cy'):
        subprocess.check_call(['git', 'clone', 'https://github.com/cyberyen/gitian.sigs.cy.git'])
...
def build():
    ...
    os.makedirs(os.path.join(workdir, 'cyberyen-binaries', args.version), exist_ok=True)
    os.chdir(os.path.join(workdir, 'gitian-builder'))
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-signed', '--destination', '../gitian.sigs.cy/', '../gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call('mv build/out/cyberyen-*win64-setup.exe ../cyberyen-binaries/'+args.version, shell=True)

    if args.macos:
        print('\nSigning ' + args.version + ' MacOS')
        subprocess.check_call('cp inputs/cyberyen-' + args.version + '-osx-unsigned.tar.gz inputs/cyberyen-osx-unsigned.tar.gz', shell=True)
        ...
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs.cy/', '../gitian-descriptors/gitian-osx-signer.yml'])
        subprocess.check_call('mv build/out/cyberyen-osx-signed.dmg ../cyberyen-binaries/'+args.version+'/cyberyen-'+args.version+'-osx.dmg', shell=True)
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.cy/', '-r', args.version+'-linux', '../gitian-descriptors/gitian-linux.yml']):
        print('Verifying v'+args.version+' Linux FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.cy/', '-r', args.version+'-win-unsigned', '../gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.cy/', '-r', args.version+'-osx-unsigned', '../gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Signed Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.cy/', '-r', args.version+'-win-signed', '../gitian-descriptors/gitian-win-signer.yml']):
        print('Verifying v'+args.version+' Signed Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.cy/', '-r', args.version+'-osx-signed', '../gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    if args.pull:
        os.chdir('../gitian-builder/inputs/cyberyen')
